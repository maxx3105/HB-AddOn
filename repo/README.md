...
4. `src/rc.d/hb-uni-sen-pool-wp-addon` → `webui_apply()` um neue `elvST`-Einträge und DEV_LIST-Eintrag erweitern
...
```bash
cd src
tar -czf ../hb-uni-sen-pool-wp-addon-1.0.tgz rc.d addon update_script
```
