#!/bin/sh
DEVLISTFILE="/usr/local/addons/hb-addon/devdb.csv"
  ...
  if case $DEVICE in "HB-"*) true;; *) false;; esac; then
    ...
    case "$1" in
        ""|install)
          ...
          if [ -z "`cat $DEVDBFILE | grep \"$DEVICE\"`" ]; then
            sed -i "s/\(array[[:space:]]*set[[:space:]]*DEV_PATHS[[:space:]]*{\)/\1$DEVDBINSERT/g" $DEVDBFILE
          fi
          ;;
        uninstall)
